...
| Kemampuan | Singly LL | Doubly LL |
|---|---|---|
| Traversal maju | ✅ | ✅ |
| Traversal mundur | ❌ | ✅ |
| Hapus node tanpa pointer prev | Butuh node sebelumnya | Langsung O(1) |
| Implementasi Deque | Sulit | Mudah |
...
1. **Deteksi overflow** — Python mendeteksi `len == capacity`
2. **Alokasi memori baru** — Python mengalokasikan blok memori baru yang lebih besar. Python menggunakan faktor pertumbuhan sekitar **1.125× hingga 2×** (bukan persis 2×, tergantung ukuran saat ini)
3. **Copy semua elemen** — Seluruh elemen lama disalin ke lokasi memori baru → ini operasi O(n)
4. **Bebaskan memori lama** — Blok memori lama di-dealokasi
5. **Sisipkan elemen baru** — Elemen baru ditambahkan di posisi berikutnya
